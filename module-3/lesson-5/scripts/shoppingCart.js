...
function getTotalPrice(arrShoppingCart=[],objCoupon={}){
  ...
  for(var counter=0; counter < arrShoppingCart.length; counter++){
    ...
    if(objCoupon && objCoupon.type == 'percentage'){
      //if the coupon category = the current item type of the coupon category = all
    ...
    //total = the old total + current item price
    total = total + itemPrice;
    // console.log(total);
  }
  //If the coupon has been passed as an argument and the coupon type is total
